...
export class FirebaseDataStore {
  ...
  fetch() {
    ...
    return new Promise((resolve, reject) => {
      this.ref.child(`recordings/${this.apiVersion}`)
        .once("value")
        .then((snapshot) => {
  ...
  storeForSeed(seed: string, data: PlayerData) {
    if (data.apiVersion != this.apiVersion) {
      throw `Attempting to store data with a different API version than the API client: ${data.apiVersion} vs ${this.apiVersion}`
    }
